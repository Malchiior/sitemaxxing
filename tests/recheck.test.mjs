...
test("the re-check text is one line: counts, scores, the PDF; the detail lines are separate", () => {
  const before = run([50, 75, 90], [logo, sideways, taps], [noDescription, noLlms]);
  const after = run([88, 75, 90], [taps], [noDescription, noLlms]);
  const text = recheckMessage(after, before);
  assert.equal(text, "sbeoc.com again (since Sep 22): fixed 2, still there 3, new 0. Phones went from 50 to 88. Tablets and computers unchanged. The PDF has what's left.");
  ...
  assert.match(recheckMessage(withPages, before), /The PDF has what's left; reply pages to check \/about and \/contact too\.$/);
...
test("the re-check text: several score groups changed, nothing fixed yet, long lists capped", () => {
  ...
  assert.match(recheckMessage(after, before), /^sbeoc\.com again \(since Sep 22\): fixed 0, still there 2, new 7\. Phones went from 50 to 30, tablets from 75 to 60\. Computers unchanged\. The PDF has what's left\.$/);
...
test("the re-check text when everything is fixed and scores didn't move", () => {
  ...
  assert.equal(text, "sbeoc.com again (since Sep 22): fixed 1, still there 0, new 0. Scores unchanged: 100 on phones, 100 on tablets, 100 on computers. Nothing left to fix on this page.");
});
