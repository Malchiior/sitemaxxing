...
/** The newest earlier check of `url` under `runsDir`, with the files a diff needs, or null. */
export function previousRun(runsDir: string, url: string): string | null {
  ...
  for (const name of readdirSync(runsDir).sort().reverse()) {
    ...
    try {
      ...
      if ([summary.requested, summary.site].filter(Boolean).some((u: string) => pageKey(u) === want)) return dir;
